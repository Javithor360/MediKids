...
export const SearchViewAppointmentHistory = ({ values }) => {
  const { AppointmentsHistory, AllDoctors, appointmentHistory, doctors } = useDash();

  useEffect(() => {
    AppointmentsHistory(values.Patient_id);
    AllDoctors();
  }, []);
  ...
  return (
    ...
    <div className="w-full h-full">
        ...
        <div className="text-[1.4rem] text-[#707070] font-bold flex flex-row items-center gap-3">{`${values.Name}`}</div>
